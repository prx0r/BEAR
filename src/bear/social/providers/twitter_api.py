"""TwitterAPI.io provider — $0.15/1k, no minimum, best default."""
...
class TwitterAPI(XProvider):
    ...
    def search(
        ...
        """Advanced search. NOTE: keep ≤20 per request due to X pagination issues."""
        params = {
            "query": query,
            "queryType": "Latest",
        }
    ...
    def user_posts(
        ...
        """Get user timeline."""
        params = {"userName": username, "queryType": "Latest"}
        if cursor:
            params["cursor"] = cursor

        resp = self._client.get("/tweet/user_timeline", params=params)
        resp.raise_for_status()
        data = resp.json()
    ...
    def get_thread(self, tweet_id: str, limit: int = 20) -> list[Tweet]:
        ...
        thread = [post]
        current = post.raw
        # Walk up the thread via in_reply_to
    ...
    def get_replies(self, tweet_id: str, limit: int = 20, cursor: str = "") -> SearchResponse:
        """Get replies via search (TwitterAPI doesn't have dedicated replies endpoint)."""
        post = self.get_post(tweet_id)
        if not post:
            return SearchResponse(tweets=[], provider=self.name)

        # Search for replies by author conversation
